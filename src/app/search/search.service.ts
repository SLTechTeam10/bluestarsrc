...
export class SearchService {
  private listingDataStore: DataStore<SearchProductModel>;
  // private detailsDataStore: DataStore<FashionDetailsModel>;

  constructor(private http: HttpClient) { }

  public getListingDataSource(): Observable<SearchProductModel> {
    return this.http.get<SearchProductModel>('./assets/sample-data/fashion/listing.json')
    .pipe(
      map(
        (data: SearchProductModel) => {
          // Note: HttpClient cannot know how to instantiate a class for the returned data
          // We need to properly cast types from json data
          const listing = new SearchProductModel();
  ...
  public getListingStore(dataSource: Observable<SearchProductModel>): DataStore<SearchProductModel> {
    // Use cache if available
    if (!this.listingDataStore) {
      // Initialize the model specifying that it is a shell model
      const shellModel: SearchProductModel = new SearchProductModel();
